$(document).ready(function () {
    ...
    for (var i = 0; i < tree.length; i++)
    {
        if (tree[i].parentID == 0)
            family.push({ id: tree[i].id, title: tree[i].title, children: [] });
    }

    for (var i = 0; i < family.length; i++) {
        $("#list").append("<li class='" + tree[i].gender + " shadow'>" + family[i].title + "<ul id='" + family[i].id + "'></ul></li>");
        family[i].children = findChildren(family[i].id);
    }
